...
enum class Format { UN_RGB8, F_RGB16, count };
...
#define R_CREATE_TEXTURE		R_DECORATE(TextureId, createTexture, (char const* path, Address address, Filter filter), (path, address, filter))
...
#define R_BIND_BUFFER			R_DECORATE(void, bindBuffer, (Stage stage, u32 slot, BufferId buffer), (stage, slot, buffer))
...
#define R_BIND_TEXTURE			R_DECORATE(void, bindTexture, (Stage stage, u32 slot, TextureId texture), (stage, slot, texture))
...
#define R_FILL_RENDER_TARGET	R_DECORATE(void, fillRenderTarget, (u32 const* data), (data))
// clang-format on
...
#undef ADD_ARG
	...
	FORCEINLINE RenderTargetId createRenderTarget(v2u size, u32 sampleCount) { return createRenderTarget(size, Format::UN_RGB8, sampleCount); }
	FORCEINLINE RenderTargetId createRenderTarget(v2u size, Format format) { return createRenderTarget(size, format, 1); }
